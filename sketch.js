...
function spawnCrows(){
  if (frameCount % 100 === 0){
   var crow = createSprite(600,Math.round(random(100,120)),10,10);
   crow.velocityX = -(6 + score/100);
    crow.addImage(crowImg);
    crow.debug = true;
    crow.setCollider("rectangle",200,0,60,60);
    crow.scale = 0.5;
    crow.lifetime = 300;
    trex.depth = crow.depth;
    trex.depth++;
    crowsGroup.add(crow);
  }
}
